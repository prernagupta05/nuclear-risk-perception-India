...
a <- castegendereffect %>%
  ...
  summarise(mean=mean(riskscore), sd=sd(riskscore), n=n(), median= median(riskscore))%>%
  arrange(desc(mean))

a %>%  ggplot(aes(y = Gender_Caste, x = mean))+
...
castegenderben %>%
  ...
  dplyr::group_by(Gender_Caste)%>%
  summarise(mean=mean(benscore), sd=sd(benscore))%>%
  arrange(desc(mean))%>%
...
cenbensummary <- joineddf %>%
  ...
  #melt to long format
  gather(key = "technology", value = "summary", Ben_Hydro_mean, Ben_Solar_mean, Ben_Wind_mean, Ben_Nuclear_mean, Ben_Coal_mean, Ben_Gas_mean, Ben_Oil_mean, Ben_Hydro_sd, Ben_Solar_sd, Ben_Wind_sd, Ben_Nuclear_sd, Ben_Coal_sd, Ben_Gas_sd, Ben_Oil_sd) %>%
  select(State, technology, summary) %>%
  # separate risky from variable suffix
