...
wilcox_gender <- do.call(rbind, wilcox_gender)
...
# for making a data frame from wilcox test
#wilcox_gender <- demovars %>% 
  #filter(gender %in% c("Male", "Female"))%>%
  #select(starts_with("Risky")) %>% 
  #lapply(function(col) broom::tidy(wilcox.test(col ~ gender, data = demovars)))

#wilcox_gender <- do.call(rbind, wilcox_gender)

#wilcox_gender$variable <- rownames(wilcox_gender)

#wilcox_gender <- as.data.frame(wilcox_gender)
```
