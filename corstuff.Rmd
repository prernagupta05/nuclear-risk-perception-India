...
# code likert scale for variables for Kahan scale into numbers
  mutate_at(vars(starts_with (c("K_H","K_I","DISPLACE", "POLLUTE", "HEALTH", "JOBS", "BEAUTY", "PRIDE", "NPRIDE","DEVNUCLEAR","PROSPER", "RELY"))), funs(case_when(. == "Strongly disagree" ~ 1, 
               . == "Somewhat disagree" ~ 2,
               . == "Neither agree nor disagree" ~3,
               . == "Somewhat agree" ~ 4,
               . == "Strongly agree" ~ 5))) %>%
...
# Demographic IVs - two linear regression models
...
#  Kahan scale Alpha values 
Hierarchy - Egalitarianism scale alpha  =  0.70
Individualism - Communitarianism scale alpha= 0.52
Only Individualism scale alpha = 0.31 
...
```{r, echo=FALSE}
## Alternative Kahan Scores with Mean across
...
fascale_scorescor <- fascale_scores %>%
  dplyr::select(KahanS, KahanH, Pdevelop, Ndevelop)

cor_kecopol1 <- cor(fascale_scorescor)
...
# Correlation Eco-pol and alt Kahan (mean across)
...
fascale_scores2cor <- fascale_scores2 %>%
  dplyr::select(Individualism_score, Hierarchy_score, Pdevelop, Ndevelop)
...
\newpage
# Scatter plot 1: mean across Kahan scale scores around medians
...
# Scater plot 2:  Kahan CFA scores 
```{r, echo = FALSE}
...
# 3) lm with kahan 
m1_kahan2 <- lm(Risky_Nuclear ~ Uppercaste + Male + Hindu + Urban + age + State + Individualism_score + Hierarchy_score , data = fascale_scores2)
# Print model summary
#summary(m1_kahan2)
#plot(m1_kahan)

# 4) lm with ecopolvars 
m1_allscales2 <- lm(Risky_Nuclear ~ Uppercaste + Male + Hindu + Urban + age + State+ Pdevelop + Ndevelop + Individualism_score + Hierarchy_score , data = fascale_scores2)
# Print model summary
#summary(m1_allscales2)
#plot(m1_allscales)
...
cat("\\setlength{\\tabcolsep}{1pt}")
cat("\\renewcommand{\\arraystretch}{0.7}")
stargazer(m1_kahan2, m1_ecopol2, m1_allscales2, 
          title = "Alternative Kahan Scores(mean acorss fixed factors)",
          type = "latex"
)
cat("\\endgroup")
...
cat("\\setlength{\\tabcolsep}{1pt}")
cat("\\renewcommand{\\arraystretch}{0.7}")
stargazer(m1_kahan,m1_ecopol, m1_allscales, 
          title = "Lms with CFA Kahan",
          type = "latex"
)
cat("\\endgroup")
...
Kahan_scale1 <- codedmodule2 %>%
  dplyr::select(starts_with(c("Risky_Nuclear","K_", "Uppercaste", "Male", "Hindu", "Urban", "State","age"))) %>%
  mutate_at(vars(age), funs(case_when(. == "18-24 years old" ~ 1, 
                                      . == "25-34 years old" ~ 2,
                                      . ==  "35-44 years old" ~3,
                                      . == "45-54 years old" ~ 4,
...
# The factor scores will be stored in the scores element of the returned list
kahanefascores <- Kahanefa$scores


fascale_scores4 <- cbind(Kahan_scale1, kahanefascores) %>%
  rename(CE = MR1)%>%
  rename(HE = MR2) #%>%
...
fa_table(kahan.fa) %>%
  #rename(Pdevelop = MR1, Ndevelop = MR2, Items= item) %>%
  flex("Kahan Exploratory Factor Analysis Table")
...
\newpage
# LMs with EFA Kahan factors 

```{r, echo=FALSE}

demoEFA <- lm(Risky_Nuclear ~ Uppercaste + Male + Hindu + Urban, data = fascale_scores4)

#summary(demoEFA)

# 2) lm with all demographic variables
demoEFA2<- lm(Risky_Nuclear ~ Uppercaste + Male + Hindu + Urban + age + State, data = fascale_scores4)
#summary(demoEFA2)

# 3) lm with kahan 
demoEFA3 <- lm(Risky_Nuclear ~ Uppercaste + Male + Hindu + Urban + age + State + CE + HE , data = fascale_scores4)
...
cat("\\setlength{\\tabcolsep}{1pt}")
cat("\\renewcommand{\\arraystretch}{0.7}")
stargazer(demoEFA,demoEFA2, demoEFA3, 
          title = "Lms with CFA Kahan",
          type = "latex"
)
cat("\\endgroup")
...
# 2. EXTRA:: Kahan scale EFA  
```{r, echo=FALSE}



Kahan_scale2<- fascales %>%
  dplyr::select(starts_with("K_"))
  
kahan2.fa <- fa(Kahan_scale2, nfactors = 2, rotate = "varimax")

print.psych(kahan2.fa, cut = 0.4, sort = TRUE)

# I don't know still doesn't make a lot of sense to me- the two factors make more sense - 1) people centered development 2) nationalist development
# So I am going to extract factor scores and bind them to the dataset

Kahanefa2 <- psych::factor.scores(Kahan_scale2, kahan2.fa)

# The factor scores will be stored in the scores element of the returned list
kahanefascores2 <- Kahanefa2$scores


fascale_scores5 <- cbind(fascales, kahanefascores2) %>%
  rename(CE = MR1)%>%
  rename(HE = MR2) #%>%
...
# 2. LMs with EFA Kahan factors 

```{r, echo=FALSE}

KEFA <- lm(Risky_Nuclear ~ Uppercaste + Male + Hindu + Urban, data = fascale_scores5)

#summary(KEFA)

# 2) lm with all demographic variables
KEFA2<- lm(Risky_Nuclear ~ Uppercaste + Male + Hindu + Urban + age + State, data = fascale_scores5)
#summary(KEFA2)

# 3) lm with kahan 
KEFA3 <- lm(Risky_Nuclear ~ Uppercaste + Male + Hindu + Urban + age + State + CE + HE , data = fascale_scores5)
...
Mah<- fascale_scores %>%
  filter(State == "Maharasthra")

Bengal <- fascale_scores %>%
  filter(State == "Bengal")
...
# 3 linear models - 1) lm with demographic variables except age and State
TM_demo <- lm(Risky_Nuclear ~ Uppercaste + Male + Hindu + Urban +age, data = TMalldemos)

summary(TM_demo)

# 3) lm with kahan 
TM1_kahan <- lm(Risky_Nuclear ~ Uppercaste + Male + Hindu + Urban + age  + KahanS + KahanH , data = TM)

# 4) lm with ecopolvars 
TM1_allscales <- lm(Risky_Nuclear ~ Uppercaste + Male + Hindu + Urban + age + Pdevelop + Ndevelop + KahanS + KahanH , data = TM)

TM1_ecopol <- lm(Risky_Nuclear ~ Uppercaste + Male + Hindu + Urban + age +  Pdevelop + Ndevelop,  data = TM)
...
# 3) lm with kahan 
TM2_kahan <- lm(Risky_Nuclear ~ Uppercaste + Male + Hindu + Urban + age  + Individualism_score + Hierarchy_score, data = TM2)

summary(TM2_kahan)

# 4) lm with ecopolvars 
TM2_allscales <- lm(Risky_Nuclear ~ Uppercaste + Male + Hindu + Urban + age + Pdevelop + Ndevelop + Individualism_score + Hierarchy_score , data = TM2)

summary(TM2_allscales)

TM2_ecopol <- lm(Risky_Nuclear ~ Uppercaste + Male + Hindu + Urban + age +  Pdevelop + Ndevelop,  data = TM2)
...
cat("\\setlength{\\tabcolsep}{1pt}")
cat("\\renewcommand{\\arraystretch}{0.7}")
stargazer(TM_demo, TM1_kahan, TM1_allscales, TM1_ecopol,
          title = "Results from 2 linear regression models",
          type = "latex"
)
cat("\\endgroup")
