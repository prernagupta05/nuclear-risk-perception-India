...
module2%>%
  ...
  group_by(Technology, Perceived_Risk) %>%
  summarise(n=n())%>%
ggplot(aes(fill= Perceived_Risk, y=n, x=Technology)) + 
  geom_bar(position="fill", stat="identity")+
  coord_flip()

# fix ugly visual. Rearrange from extremely risky to not at all risky. Red to green, rather not say/Don't know should be grey 
...
module2%>%
  ...
  summarise(n=n())%>%
ggplot(aes(fill= Perceived_Benefit, y=n, x=Technology)) + 
  geom_bar(position="fill", stat="identity")+
  coord_flip()

# fix ugly visual. Rearrange from extremely beneficial to not at all beneficial. Green to red, rather not say/Don't know should be grey. Also facet wrap by INDividually owned and centrally owned technology. 
