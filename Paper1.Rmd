...
Indian state regards nuclear energy as an important solution to rising energy needs and climate change issues. However, throughout India, nuclear power initiatives have faced opposition from people. Using survey data this study reveals that, similar to global trends, nuclear energy is perceived as highly risk in India as well. It is also perceived as the riskiest technology among all large-scale energy sources (like coal, solar, wind, oil gas, and hydro). Using risk perception theories and frameworks, this paper delves into the underlying factors influencing India's public perception of nuclear energy. While consistent demographic patterns, termed the "white male effect," influence risk perceptions in the US, demographic factors such as gender and caste have minimal influence on India's nuclear energy risk perceptions. Cultural paradigms like hierarchy versus egalitarianism and individualism versus communitarianism also show negligible impact. However, regional differences among states and economic and political values have significant impact on risk perceptions related to nuclear energy in the Indian context.
...
# code likert scale for variables for Kahan scale into numbers
  mutate_at(vars(starts_with (c("K_S","K_E","DISPLACE", "POLLUTE", "HEALTH", "JOBS", "BEAUTY", "PRIDE", "NPRIDE","DEVNUCLEAR","PROSPER", "RELY"))), funs(case_when(. == "Strongly disagree" ~ 1, 
               . == "Somewhat disagree" ~ 2,
               . == "Neither agree nor disagree" ~3,
               . == "Somewhat agree" ~ 4,
               . == "Strongly agree" ~ 5))) %>%
...
# Ns for states in joineddf
```{r}

statens <-  joineddf %>% dplyr::select(Risky_Nuclear, caste, gender, religion, urban_rural, age, State) %>%
...
# Nuclear is seen as riskier than others -this paper explore this risk perception
...
# Split the data into Risky and Ben subsets
risky_data <- joineddf_subset_cor %>% dplyr::select(starts_with("Risky"))
ben_data <- joineddf_subset_cor %>% dplyr::select(starts_with("Ben"))

# Rename the columns
names(risky_data) <- gsub("^Risky_", "Perceived Risk (", names(risky_data))
names(risky_data) <- paste0(names(risky_data), ")")
names(ben_data) <- gsub("^Ben_", "Perceived Benefit (", names(ben_data))
names(ben_data) <- paste0(names(ben_data), ")")

# Compute the correlation between Risky and Ben variables
correlation_matrix <- cor(risky_data, ben_data)

correlation_df <- as.data.frame(correlation_matrix)

kable_table <- kable(correlation_df, caption = " Table 8 : Correlation Table between Perceived Risk and Perceived Benefit Variables", format = "html", digits = 2) %>%
  kable_styling(full_width = F) %>%
  column_spec(1, background = "white") %>%
  add_header_above(c(" " = 1, "Perceived Benefits" = ncol(correlation_df)))%>%
  pack_rows("Perceived Risks", 1, nrow(correlation_df))

# Apply alternating background colors for columns
for (i in seq(2, ncol(correlation_df), by = 2)) {
  kable_table <- kable_table %>% 
row_spec(i, background = "#F0F0F0")
}
...
print.psych(kahan.fa, sort = TRUE, cut=0.4)

KahanHE<-Kahan_scale %>%
  dplyr::select(starts_with(c("K_H", "K_E")))

KahanHE_alpha<- psych::alpha(KahanHE, check.keys = TRUE) # alpha = 0.7

KahanSI<-Kahan_scale %>%
  dplyr::select(starts_with(c("K_S", "K_I")))

KahanSI_alpha<- psych::alpha(KahanSI, check.keys = TRUE) # alpha =0.52

KahanS<-Kahan_scale %>%
  dplyr::select(starts_with(c("K_S")))

KahanS_alpha<- psych::alpha(KahanS, check.keys = TRUE) # alpha = 0.72


KahanI<-Kahan_scale %>%
  dplyr::select(starts_with(c("K_I")))

KahanI_alpha<- psych::alpha(KahanI, check.keys = TRUE) # alpha = 0.31 
...
relevant <- codedmodule2 %>%
  dplyr::select(starts_with(c("Risky", "Ben","K_", "WEALTHLIM","MECHANISATION","DECISIONDECEN","DECISIONCEN","INDUSTRYLARGE","INDUSTRYSMALL","ECONOMYGLOBAL","ECONOMYLOCAL", "ENVOVERDEV","DEVOVERENV","OWNERPVT","OWNERNOREG","OWNERPUB", "OWNERREG","DISPLACENUCLEAR",  "POLLUTENUCLEAR", "HEALTHNUCLEAR", "JOBSNUCLEAR", "BEAUTYNUCLEAR", "PRIDENUCLEAR", "NPRIDENUCLEAR", "DEVNUCLEAR", "PROSPERNUCLEAR", "RELYNUCLEAR", "Uppercaste", "Male", "Hindu", "Urban", "State","age")))%>%
  dplyr::select(!starts_with(c("Risky_IND","Ben_IND")))%>%
  mutate_at(vars(age), funs(case_when(. == "18-24 years old" ~ 1, 
                                      . == "25-34 years old" ~ 2,
                                      . ==  "35-44 years old" ~3,
                                      . == "45-54 years old" ~ 4,
...
relevant_na <- relevant %>% summarise_all(~sum(is.na(.)))
...
\newpage
## Kahan scater plot from the CFA scores 
```{r, echo = FALSE}

Kahan_Kscores %>%
  ggplot(aes(KahanS, KahanH))+
  geom_count(aes(color = ..n.., size = ..n..))+
  guides(color = 'legend')+
  geom_hline(yintercept=0, colour="black", lwd=0.5)+
  geom_vline(xintercept=0, colour="black", lwd=0.5)+
  labs(x = "Individualism to Communitarianism", y = "Hierarchy to Egalitarianism")+
  ylim(-2,3)+
  xlim(-2,3)+
  theme_minimal() 
```
...
## alternative Kahan scores
```{r, echo=FALSE}

Kscalescores <- fascales %>%
  dplyr::select(!starts_with(("K_I")))%>%
  rowwise()%>%
  dplyr::mutate(Communitarianism_score = mean(c_across(K_SHARM:K_SPROTECT)))%>%
  dplyr::mutate(Egalitarianism_score = mean(c_across(K_HEQUAL:K_ERADEQ2)))%>%
  dplyr::select(!starts_with(c("K_I","K_S", "K_H", "K_E")))

```


# EDIT : Scatter plot of Kahan scale scores around the median scores on Individualism and Hierarchy scales
...
Kscalescores %>%
  ggplot(aes(Communitarianism_score,Egalitarianism_score))+
  geom_count(aes(color = ..n.., size = ..n..))+
  guides(color = 'legend')+
  geom_hline(yintercept=3, colour="black", lwd=1)+
  geom_vline(xintercept=3.5, colour="black", lwd=1)+
  labs(x = "Individualism to Communitarianism", y = "Hierarchy to Egalitarianism")+
  ylim(2,6)+
  xlim(2,6)+
  theme_minimal() 
...
params1$Scale <- NA
params1$Scale[1:3] <- "Communitarian"
...
print.psych(ecopolall2.fa, sort = TRUE, cut = 0.4)
...
create_beautiful_radarchart <- function(data, color = "#00AFBB", 
  ...
  radarchart(
    ...
    centerzero=FALSE,
    # Customize the grid
    cglcol = "black", cglty = 1.5, cglwd = 1.5,
    # Customize the axis
    axislabcol = "black", 
    ...
    seg = 6, caxislabels = caxislabels, title = title, 
    # Increase title size
    cex.main = 2,
    ...
  )
...
fa_table <- function(x) {
  ...
  #supress loadings
  #loadings[loadings < cut] <- ""  # alternative loadings > -cut & loadings < cut
  #get additional info  
...
ndevelop <- ecopolall %>%
  dplyr::select( "PRIDENUCLEAR", "NPRIDENUCLEAR", "DEVNUCLEAR", "PROSPERNUCLEAR")            
...
data <- tibble(
  ...
  Code = c(
    ...
    "National pride(nuclear)", "Local prosperity(nuclear)"
  ),
...
m1_allscales <- lm(Risky_Nuclear ~ Uppercaste + Male + Hindu + Urban + age + State+ Pdevelop + Ndevelop + KahanS + KahanH , data = fascale_scores)
# Print model summary
summary(m1_allscales)
#plot(m1_allscales)


m1_ecopol <- lm(Risky_Nuclear ~ Uppercaste + Male + Hindu + Urban + age + State + Pdevelop + Ndevelop,  data = fascale_scores)

summary(m1_ecopol)

m1_allscales2 <- lm(Risky_Nuclear ~ Uppercaste + Male + Hindu + Urban + age + KahanS*State + KahanH*State + Pdevelop*State + Ndevelop*State , data = fascale_scores)

summary(m1_allscales2)



state_counts2 <- fascale_scores %>%
  group_by(State) %>%
  summarise(Count = n())
...
# Lms with alt Kahan scores 
```{r, echo = FALSE}

# binding ecopol with alternative Kahan scale score 
fascale_scores2 <-cbind(Kscalescores, scores1)  %>%
  rename(Pdevelop = MR1)%>%
  rename(Ndevelop = MR2)



fascale_scores2cor <- fascale_scores2 %>%
  dplyr::select(Communitarianism_score, Egalitarianism_score, Pdevelop, Ndevelop)

cor_kecopol <- cor(fascale_scores2cor)



# 3) lm with kahan 
m1_kahan2 <- lm(Risky_Nuclear ~ Uppercaste + Male + Hindu + Urban + age + State + Communitarianism_score + Egalitarianism_score , data = fascale_scores2)
# Print model summary
summary(m1_kahan2)
#plot(m1_kahan)

# 4) lm with ecopolvars 
m1_allscales2 <- lm(Risky_Nuclear ~ Uppercaste + Male + Hindu + Urban + age + State+ Pdevelop + Ndevelop + Communitarianism_score + Egalitarianism_score , data = fascale_scores2)
# Print model summary
summary(m1_allscales2)
#plot(m1_allscales)


m1_ecopol2 <- lm(Risky_Nuclear ~ Uppercaste + Male + Hindu + Urban + age + State + Pdevelop + Ndevelop,  data = fascale_scores2)
...
sim_slopes(m1_allscales2, pred = Ndevelop, modx = State)

ggplot(fascale_scores, aes(x = Ndevelop, y = Risky_Nuclear, color = State)) +
  geom_point(alpha = 0.5) +  # Add points with some transparency
  geom_smooth(method = 'lm', se = FALSE) +  # Add linear regression lines
  theme_minimal() +
  labs(title = "Interaction Plot",
       x = "National Development (Ndevelop)",
       y = "Perceived Risk (Risky_Nuclear)")
library(car)
vif(m1_allscales)
...
ggplot(fascale_scores, aes(x = Pdevelop, y = Risky_Nuclear)) +
  ...
  labs(title = "Risky Nuclear vs Pdevelop by State",
       x = "Pdevelop",
       y = "Risky Nuclear") +
  theme_minimal()


ggplot(fascale_scores, aes(x = Ndevelop, y = Risky_Nuclear)) +
  geom_jitter(alpha = 0.5, width = 0.2) +
  #facet_wrap(~ State) +
  geom_smooth(method = "lm", se = FALSE) +
  labs(title = "Relationship between Ndevelop and Risky Nuclear",
       x = "Ndevelop",
       y = "Risky Nuclear") +
  theme_minimal()
...
ggplot(fascale_scores, aes(x = State, y = Risky_Nuclear)) +
  geom_boxplot() +
  #facet_wrap(~ State) +
  theme_minimal()
```
...
cat("\\setlength{\\tabcolsep}{1pt}")
cat("\\renewcommand{\\arraystretch}{0.7}")
stargazer(m1_ecopol, m1_allscales, 
          title = "Results from 2 linear regression models",
          type = "latex"
)
cat("\\endgroup")
...
fascale_scores1 <- fascale_scores %>%
  mutate_at(vars(Risky_Nuclear), as.factor)

## one at a time, table 
#lapply(fascale_scores1[, c("Risky_Nuclear", "Uppercaste", "Male", "Hindu","Urban", "State") ], table)
## three way cross tabs (xtabs) and flatten the table
#ftable(xtabs(~ Uppercaste + Risky_Nuclear + Male, data = fascale_scores1))
## three way cross tabs (xtabs) and flatten the table
#ftable(xtabs(~ State + Risky_Nuclear + Urban, data = fascale_scores1))

logm1= polr(Risky_Nuclear ~ Uppercaste + Male + Hindu + Urban +age + KahanS + KahanH + Pdevelop + Ndevelop + State  , data = fascale_scores1, Hess = TRUE)
#summary(logm1)

ctable1 <- coef(summary(logm1))
...
pvalue <- as_tibble(ctable1) %>%
  dplyr::select("p value")%>%
  slice(1:(n() - 4))

ci <- confint(logm1)# default method gives profiled CIs

#confint.default(logm1) # CIs assuming normality

## odds ratios
#exp(coef(logm1))

## OR and CI
ORtable <- cbind("Odds Ratio" = coef(logm1), ci, pvalue) %>%
  mutate(
...
ORtableout <- knitr::kable(ORtable, booktabs = TRUE, caption = "Odds Ratio for Perceived Risk from Nuclear Energy") %>%
  kable_styling(latex_options = c("striped", "hold_position"))
