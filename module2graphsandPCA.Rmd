...
1. Plot the  mean risk scores (Q1)as histogram ordered from high to low risk – so basically the red bars only in reverse order (aka nuclear as high risk).
...
levels(center_barplot$level)
 ...
 center_barplot%>% 
  ggplot(aes(x=technology, y= mean, fill =  center_barplot$level))+
...
Solar <- codedmodule2 %>%
  select("DISPLACESOLAR":"RELYSOLAR") 
...
ROOFS<- codedmodule2 %>%
  select("DISPLACEROOFS":"RELYROOFS")
...
FIREWOOD<- codedmodule2 %>%
  select("DISPLACEFIRE":"RELYFIRE")%>%
  pivot_longer(cols ="DISPLACEFIRE":"RELYFIRE", names_to = "Var", values_to = "score") %>%
  separate(Var, c("prefix", "Var", "technology"), sep = c(0,-8), remove = T)%>%
  select(!"prefix")


COAL<- codedmodule2 %>%
  select("DISPLACECOAL":"RELYCOAL")%>%
  pivot_longer(cols ="DISPLACECOAL":"RELYCOAL", names_to = "Var", values_to = "score") %>%
  separate(Var, c("prefix", "Var", "technology"), sep = c(0,-4), remove = T)%>%
  select(!"prefix")


NUCLEAR<- codedmodule2 %>%
  select("DISPLACENUCLEAR":"RELYNUCLEAR")%>%
  pivot_longer(cols ="DISPLACENUCLEAR":"RELYNUCLEAR", names_to = "Var", values_to = "score") %>%
  separate(Var, c("prefix", "Var", "technology"), sep = c(0,-7), remove = T)%>%
  select(!"prefix")
